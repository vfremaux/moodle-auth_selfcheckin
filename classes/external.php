 ...
 * @since      Moodle 3.2
 */
class auth_selfcheckin_external extends external_api {
    ...
    protected static function check_signup_enabled() {
        ...
        if (empty($CFG->registerauth) or $CFG->registerauth != 'selfcheckin') {
    ...
    public static function signup_user($username, $password, $firstname, $lastname, $email, $city = '', $country = '',
                                        $customprofilefields = array(), $redirect = '') {
        global $CFG, $PAGE;
        ...
        if (!empty($errors)) {
            ...
            $authplugin->user_signup_internal($user, $password);
